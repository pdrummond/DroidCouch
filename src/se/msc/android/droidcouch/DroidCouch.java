...
public class DroidCouch {
	 ...
	 */
	public static String createDocument(String hostUrl, String db, String docId, JSONObject jsonDoc) {
		try {
			HttpPut httpPutRequest = new HttpPut(hostUrl + "/" + db + "/"
					+ docId);
	...
	public static boolean deleteDocument(String hostUrl, String databaseName,
		...
		try {
			...
			Log.e(TAG, "Error deleting couchdb document", e);
		}
		return false;
	}
	...
	public static boolean deleteDocument(String hostUrl, String databaseName,
		...
		try {
			...
			Log.e(TAG, "Error deleting couchdb document (with rev)", e);
		}
	...
	/**
	 * @return the result of the query as a JSONObject or null if there was an error.
	 */
	public static JSONObject queryView(String hostUrl, String db, String viewName) {
		String[] designAndView = viewName.split("/");
		if(designAndView.length == 2) { 
			HttpGet httpGetRequest = new HttpGet(hostUrl + "/" + db + "/_design" + "/" + designAndView[0] + "/_view/" + designAndView[1] );
			JSONObject jsonResult = sendCouchRequest(httpGetRequest);
			if (jsonResult != null) {
				return jsonResult;
			}
		} else {
			Log.e(TAG, "viewName must be of the format design/view");
		}
		return null;
	}

	public static boolean replicateDb(String hostUrl, String source, String target, boolean continuous) {
		return replicateDb(hostUrl, source, target, continuous, null, null);
	}
	
	public static boolean replicateDb(String hostUrl, String source, String target, boolean continuous, String filter, JSONObject queryParams) {
		boolean result = false;
		try {
			HttpPost req = new HttpPost(hostUrl + "/_replicate");                   
			...
			Log.d(TAG, "Replication request: " + json.toString());
			StringEntity body = new StringEntity(json.toString(), "utf8");
			req.setEntity(body);
			req.setHeader("Accept", "application/json");
			req.setHeader("Content-type", "application/json");
			JSONObject jsonResult = sendCouchRequest(req);
			
			Log.d(TAG, "Replication result: " + jsonResult);
			result  = jsonResult.getBoolean("ok");
			
		} catch (Exception e) {
			Log.e(TAG, "Error in replicateDb", e);
		}
		return result;
	}
	...
	private static JSONObject sendCouchRequest(HttpUriRequest request) {
		try {
			Log.d(TAG, "Sending Couch Request: " + request.getURI());
	...
	public static String updateDocument(String hostUrl, String databaseName,
			JSONObject jsonDoc) {
		try {
			Log.d(TAG, "updateDocument: " + jsonDoc);
			...
			Log.e(TAG, "Error updating doc: ", e);
		}
		return null;
	}
	...
	public static JSONObject getAllDocuments(String hostUrl, String databaseName) {
		try {
			String url = hostUrl + databaseName + "/_all_docs?include_docs=true";
